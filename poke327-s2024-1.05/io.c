...
void io_display() {
                ...
                for (x = 0; x < MAP_X; x++) {
                        if (world.cur_map->cmap[y][x]) {
                                ...
                                case ter_water:
                                        ...
                                        break;
                                default:
                                        /* Use zero as an error symbol, since it
                                         * stands out somewhat, and it's * not
                                         * otherwise used. */
                                        attron(COLOR_PAIR(COLOR_CYAN));
        ...
        if ((c = io_nearest_visible_trainer())) {
                attron(COLOR_PAIR(COLOR_RED));
                mvprintw(
                    22, 55, "%c at vector %d%cx%d%c.", c->symbol,
...
static void io_list_trainers_display(character_t **c, uint32_t count) {
        ...
        snprintf(s[0], TRAINER_LIST_FIELD_WIDTH,
                 "You know of %d trainers:", count);
        mvprintw(4, 19, " %-40s ", *s);
        mvprintw(5, 19, " %-40s ", "");

        for (i = 0; i < count; i++) {
                snprintf(
                    s[i], TRAINER_LIST_FIELD_WIDTH, "%16s %c: %2d %s by %2d %s",
                    char_type_name[c[i]->npc->ctype], c[i]->symbol,
                    abs(c[i]->pos[dim_y] - world.pc.pos[dim_y]),
                    ((c[i]->pos[dim_y] - world.pc.pos[dim_y]) <= 0 ? "North"
                                                                   : "South"),
                    abs(c[i]->pos[dim_x] - world.pc.pos[dim_x]),
                    ((c[i]->pos[dim_x] - world.pc.pos[dim_x]) <= 0 ? "West"
                                                                   : "East"));
                if (count <= 13) {
...
void io_pokemart() {
        mvprintw(0, 0,
                 "Welcome to the Pokemart.  Could I interest you in some "
                 "Pokeballs?");
        refresh();
        getch();
}
...
void io_battle(character_t *aggressor, character_t *defender) {
        ...
        io_display();
        mvprintw(0, 0,
                 "Aww, how'd you get so strong?  You and your pokemon must "
...
uint32_t move_pc_dir(uint32_t input, pair_t dest) {
        ...
        case 9:
                ...
                break;
        case '>':
                if (world.cur_map->map[world.pc.pos[dim_y]]
                                      [world.pc.pos[dim_x]] == ter_mart) {
                        io_pokemart();
                }
                if (world.cur_map->map[world.pc.pos[dim_y]]
                                      [world.pc.pos[dim_x]] == ter_center) {
                        io_pokemon_center();
                }
                break;
        }
...
void io_handle_input(pair_t dest) {
        ...
        do {
                ...
                case 'q':
                        /* Demonstrate use of the message queue.  You can use
                         * this for * printf()-style debugging (though gdb is
                         * probably a better   * option.  Not that it matters,
                         * but using this command will   * waste a turn.  Set
                         * turn_not_consumed to 1 and you should be * able to
                         * figure out why I did it that way. */
                        io_queue_message("This is the first message.");
                        io_queue_message("Since there are multiple messages, "
                                         "you will see \"more\" prompts.");
                        io_queue_message(
                            "You can use any key to advance through messages.");
                        io_queue_message(
                            "Normal gameplay will not resume until the queue "
                            "is empty.");
                        io_queue_message(
                            "Long lines will be truncated, not wrapped.");
                        io_queue_message(
                            "io_queue_message() is variadic and handles "
                            "all printf() conversion specifiers.");
                        io_queue_message("Did you see %s?", "what I did there");
                        io_queue_message(
                            "When the last message is displayed, there will "
                            "be no \"more\" prompt.");
                        ...
                        break;
                default:
                        /* Also not in the spec.  It's not always easy to figure
                         * out what * key code corresponds with a given
                         * keystroke.  Print out any    * unhandled key here.
                         * Not only does it give a visual error      *
                         * indicator, but it also gives an integer value that
                         * can be used * for that key in this (or other) switch
                         * statements.  Printed in * octal, with the leading
                         * zero, because ncurses.h lists codes in * octal, thus
                         * allowing us to do reverse lookups.  If a key has a *
                         * name defined in the header, you can use the name
                         * here, else    * you can directly use the octal value.
                         */
